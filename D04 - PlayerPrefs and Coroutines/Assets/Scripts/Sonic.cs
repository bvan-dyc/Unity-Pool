...
{
	[HideInInspector] public int			rings = 0;
	[HideInInspector] public int deathCount = 0;
	public float		speedFactor;
	[HideInInspector]public float		speed;
	public int hitImpactForce = 3;
	...
	public void getHit() {
		if (isInvincible == true)
			return;
		if (rings == 0 && !isShielded)
			dead();
		else
		{
			Vector2 hitDirection = new Vector2 (rbody.velocity.normalized.x, rbody.velocity.normalized.y);
			rbody.velocity = new Vector2(0, 0);
			rbody.AddForce(-hitDirection * hitImpactForce * (isShielded ? 0.8f : 1f), ForceMode2D.Impulse);
			isHit = true;
			Invoke("stopHit", 2);
